...
namespace EloFactory_TwistedFate
{
    internal class Program
    {
        ...
        {
            Player.SetSkin(Player.BaseSkinName, Config.Item("TwistedFate.SkinChanger").GetValue<bool>() ? Config.Item("TwistedFate.SkinChangerName").GetValue<StringList>().SelectedIndex : Player.BaseSkinId);
        ...
        public static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (Config.Item("TwistedFate.HarassActive").GetValue<KeyBind>().Active || Config.Item("TwistedFate.HarassActiveT").GetValue<KeyBind>().Active)
            {
                if (Config.Item("TwistedFate.QWhenEnemyCastHarass").GetValue<bool>() && (unit.IsValid<Obj_AI_Hero>() && !unit.IsValid<Obj_AI_Turret>()) && unit.IsEnemy && args.Target.IsMe && Q.IsReady() && Player.Distance(unit) <= Q.Range)
                {
